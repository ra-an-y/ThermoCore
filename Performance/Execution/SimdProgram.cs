...
namespace ThermoCore.Performance.ReferenceCpuSimd
{
    internal static class SimdProgram
    {
        ...
        private static int Main()
        {
            try
            {
                ...
                Console.WriteLine("SIMD performance evaluation: COMPLETED — measurements reported.");
                return 0;
            }
            catch (Exception ex)
            {
                Console.Error.WriteLine(
                    $"SIMD performance evaluation: INVALID — {ex.GetType().Name}: {ex.Message}");
                return 1;
            }
        }
        ...
        private static CompiledThermodynamicParameters BuildMaterial()
        {
            var definition = new ReferenceMaterialDefinition(
                materialId: "performance-simd-synthetic-reference-v0.1",
                provenance: "Synthetic fixed configuration for SIMD performance evaluation only",
            ...
            CompiledThermodynamicParameters material)
        {
            ...
            var enthalpies = CreateEnthalpies(count, material);
            ...
            RecoveryScenario scenario)
        {
            ...
            var enthalpies = CreateEnthalpies(cellCount, material);
        ...
        private static double[] CreateEnthalpies(
            int count,
            CompiledThermodynamicParameters material)
        {
            ...
            for (var pass = 0; pass < passes; pass++)
            {
                for (var i = 0; i < enthalpies.Length; i++)
                {
                    var derived = ReferenceThermodynamicFormulation.Recover(
                        new ThermodynamicState(enthalpies[i]),
                        material);
        ...
        {
            Console.WriteLine("Reference CPU SIMD performance evaluation");
